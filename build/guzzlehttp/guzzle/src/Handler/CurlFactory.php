...
namespace BackdropS3FS\GuzzleHttp\Handler;

use BackdropS3FS\GuzzleHttp\Exception\ConnectException;
use BackdropS3FS\GuzzleHttp\Exception\RequestException;
use BackdropS3FS\GuzzleHttp\Promise as P;
use BackdropS3FS\GuzzleHttp\Promise\FulfilledPromise;
use BackdropS3FS\GuzzleHttp\Promise\PromiseInterface;
use BackdropS3FS\GuzzleHttp\Psr7\LazyOpenStream;
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private static function createRejection(EasyHandle $easy, array $ctx): PromiseInterface
    {
        ...
        if ('' !== $sanitizedError) {
            $redactedUriString = \BackdropS3FS\GuzzleHttp\Psr7\Utils::redactUserInfo($uri)->__toString();
    ...
    private static function sanitizeCurlError(string $error, UriInterface $uri): string
    {
        ...
        $redactedUriString = \BackdropS3FS\GuzzleHttp\Psr7\Utils::redactUserInfo($baseUri)->__toString();
    ...
    private function applyHandlerOptions(EasyHandle $easy, array &$conf): void
    {
        ...
        if (!isset($options['sink'])) {
            ...
            $options['sink'] = \BackdropS3FS\GuzzleHttp\Psr7\Utils::tryFopen('php://temp', 'w+');
        }
        $sink = $options['sink'];
        if (!\is_string($sink)) {
            $sink = \BackdropS3FS\GuzzleHttp\Psr7\Utils::streamFor($sink);
