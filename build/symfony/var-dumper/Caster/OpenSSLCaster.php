...
final class OpenSSLCaster
{
    public static function castOpensslX509(\OpenSSLCertificate $h, array $a, Stub $stub, bool $isNested): array
    {
        ...
        $a += [Caster::PREFIX_VIRTUAL . 'subject' => new EnumStub(array_intersect_key($info['subject'], ['organizationName' => \true, 'commonName' => \true])), Caster::PREFIX_VIRTUAL . 'issuer' => new EnumStub(array_intersect_key($info['issuer'], ['organizationName' => \true, 'commonName' => \true])), Caster::PREFIX_VIRTUAL . 'expiry' => new ConstStub(date(\DateTimeInterface::ISO8601, $info['validTo_time_t']), $info['validTo_time_t']), Caster::PREFIX_VIRTUAL . 'fingerprint' => new EnumStub(['md5' => new ConstStub(wordwrap(strtoupper(openssl_x509_fingerprint($h, 'md5')), 2, ':', \true)), 'sha1' => new ConstStub(wordwrap(strtoupper(openssl_x509_fingerprint($h, 'sha1')), 2, ':', \true)), 'sha256' => new ConstStub(wordwrap(strtoupper(openssl_x509_fingerprint($h, 'sha256')), 2, ':', \true)), 'pin-sha256' => new ConstStub($pin)])];
        return $a;
    }
