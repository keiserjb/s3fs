...
class RedisCaster
{
    ...
    {
        $serializer = $redis->getOption(\defined('BackdropS3FS\Redis::OPT_SERIALIZER') ? \Redis::OPT_SERIALIZER : 1);
        ...
        $compression = \defined('BackdropS3FS\Redis::OPT_COMPRESSION') ? $redis->getOption(\Redis::OPT_COMPRESSION) : 0;
        ...
        $retry = \defined('BackdropS3FS\Redis::OPT_SCAN') ? $redis->getOption(\Redis::OPT_SCAN) : 0;
        if (\is_array($retry)) {
            foreach ($retry as &$v) {
        ...
        $options += ['TCP_KEEPALIVE' => \defined('BackdropS3FS\Redis::OPT_TCP_KEEPALIVE') ? $redis->getOption(\Redis::OPT_TCP_KEEPALIVE) : Relay::OPT_TCP_KEEPALIVE, 'READ_TIMEOUT' => $redis->getOption(\defined('BackdropS3FS\Redis::OPT_READ_TIMEOUT') ? \Redis::OPT_READ_TIMEOUT : Relay::OPT_READ_TIMEOUT), 'COMPRESSION' => $compression, 'SERIALIZER' => $serializer, 'PREFIX' => $redis->getOption(\defined('BackdropS3FS\Redis::OPT_PREFIX') ? \Redis::OPT_PREFIX : Relay::OPT_PREFIX), 'SCAN' => $retry];
