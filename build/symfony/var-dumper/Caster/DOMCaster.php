...
class DOMCaster
{
    ...
    public static function castException(\DOMException|\BackdropS3FS\Dom\Exception $e, array $a, Stub $stub, bool $isNested): array
    {
    ...
    {
        foreach ($a as $k => $v) {
            if ('encoding' === $k && $dom instanceof \DOMEntity || \in_array($k, ['actualEncoding', 'config', 'standalone', 'version'], \true)) {
                continue;
            ...
            $a[$k] = match (\true) {
                ...
                'baseURI' === $k && $v, 'documentURI' === $k && $v => new LinkStub($v),
                default => $v,
            };
        }
