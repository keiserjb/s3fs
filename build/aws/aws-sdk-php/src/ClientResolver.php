...
use BackdropS3FS\Aws\Api\ApiProvider;
use BackdropS3FS\Aws\Api\Service;
use BackdropS3FS\Aws\Api\Validator;
use BackdropS3FS\Aws\Auth\AuthResolver;
use BackdropS3FS\Aws\Auth\AuthSchemeResolver;
use BackdropS3FS\Aws\Auth\AuthSchemeResolverInterface;
use BackdropS3FS\Aws\ClientSideMonitoring\ApiCallAttemptMonitoringMiddleware;
use BackdropS3FS\Aws\ClientSideMonitoring\ApiCallMonitoringMiddleware;
use BackdropS3FS\Aws\ClientSideMonitoring\Configuration;
use BackdropS3FS\Aws\Configuration\ConfigurationResolver;
use BackdropS3FS\Aws\Credentials\CredentialProvider;
use BackdropS3FS\Aws\Credentials\Credentials;
use BackdropS3FS\Aws\Credentials\CredentialsInterface;
use BackdropS3FS\Aws\DefaultsMode\ConfigurationInterface as ConfigModeInterface;
use BackdropS3FS\Aws\DefaultsMode\ConfigurationProvider as ConfigModeProvider;
use BackdropS3FS\Aws\Endpoint\EndpointProvider;
use BackdropS3FS\Aws\Endpoint\PartitionEndpointProvider;
use BackdropS3FS\Aws\Endpoint\UseDualstackEndpoint\Configuration as UseDualStackEndpointConfiguration;
use BackdropS3FS\Aws\Endpoint\UseDualstackEndpoint\ConfigurationInterface as UseDualStackEndpointConfigurationInterface;
use BackdropS3FS\Aws\Endpoint\UseDualstackEndpoint\ConfigurationProvider as UseDualStackConfigProvider;
use BackdropS3FS\Aws\Endpoint\UseFipsEndpoint\Configuration as UseFipsEndpointConfiguration;
use BackdropS3FS\Aws\Endpoint\UseFipsEndpoint\ConfigurationInterface as UseFipsEndpointConfigurationInterface;
use BackdropS3FS\Aws\Endpoint\UseFipsEndpoint\ConfigurationProvider as UseFipsConfigProvider;
use BackdropS3FS\Aws\EndpointDiscovery\ConfigurationInterface;
use BackdropS3FS\Aws\EndpointDiscovery\ConfigurationProvider;
use BackdropS3FS\Aws\EndpointV2\EndpointDefinitionProvider;
use BackdropS3FS\Aws\Exception\AwsException;
use BackdropS3FS\Aws\Exception\InvalidRegionException;
use BackdropS3FS\Aws\Retry\ConfigurationInterface as RetryConfigInterface;
use BackdropS3FS\Aws\Retry\ConfigurationProvider as RetryConfigProvider;
use BackdropS3FS\Aws\Signature\SignatureProvider;
use BackdropS3FS\Aws\Token\Token;
use BackdropS3FS\Aws\Token\TokenInterface;
use BackdropS3FS\Aws\Token\BedrockTokenProvider;
use BackdropS3FS\Aws\Token\TokenProvider;
use BackdropS3FS\GuzzleHttp\Promise\PromiseInterface;
...
class ClientResolver
{
                ...
                if (isset($a['default'])) {
                    ...
                    if (is_callable($a['default']) && (is_array($a['default']) || $a['default'] instanceof \Closure)) {
                        if ($a['default'] === self::DEFAULT_FROM_ENV_INI) {
                            $args[$key] = $a['default']($key, $a['valid'][0] ?? 'string', $args);
                        } else {
    ...
    public static function _apply_disable_request_compression($value, array &$args)
    {
        ...
        if (!is_bool($value)) {
            throw new IAE("Invalid configuration value provided for 'disable_request_compression'." . " value must be a bool.");
        }
    ...
    public static function _apply_min_compression_size($value, array &$args)
    {
        ...
        if (!is_int($value) || is_int($value) && ($value < 0 || $value > 10485760)) {
            throw new IAE(" Invalid configuration value provided for 'min_compression_size_bytes'." . " value must be an integer between 0 and 10485760, inclusive.");
        }
        $args['config']['request_min_compression_size_bytes'] = $value;
    }
    public static function _default_min_compression_size(array &$args)
    {
        return ConfigurationResolver::resolve('request_min_compression_size_bytes', 10240, 'int', $args);
    }
    public static function _apply_credentials($value, array &$args)
    {
        ...
        if ($value instanceof CredentialsInterface) {
            ...
            throw new IAE('Credentials must be an instance of ' . "'" . CredentialsInterface::class . ', an associative ' . 'array that contains "key", "secret", and an optional "token" ' . 'key-value pairs, a credentials provider function, or false.');
        }
    }
    ...
    public static function _apply_token($value, array &$args)
    {
        ...
        } elseif (is_array($value) && isset($value['token'])) {
            $args['token'] = TokenProvider::fromToken(new Token($value['token'], $value['expires'] ?? null));
        } elseif ($value instanceof CacheInterface) {
            $args['token'] = TokenProvider::defaultProvider($args);
        } else {
            throw new IAE('Token must be an instance of ' . TokenInterface::class . ', an associative ' . 'array that contains "token" and an optional "expires" ' . 'key-value pairs, a token provider function, or false.');
        }
    }
    ...
    {
        if ($value === \false) {
            $value = new Configuration(\false, \BackdropS3FS\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_HOST, \BackdropS3FS\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_PORT, \BackdropS3FS\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_CLIENT_ID);
            $args['csm'] = $value;
        }
    ...
    public static function _apply_endpoint_provider($value, array &$args)
    {
        if (!isset($args['endpoint'])) {
            if ($value instanceof \BackdropS3FS\Aws\EndpointV2\EndpointProviderV2) {
            ...
            if (isset($serviceEndpoints[$args['region']]['deprecated'])) {
                trigger_error("The service " . $args['service'] . "has " . " deprecated the region " . $args['region'] . ".", \E_USER_WARNING);
            }
            $args['region'] = \BackdropS3FS\Aws\strip_fips_pseudo_regions($args['region']);
            ...
            if (empty($args['config']['signature_version'])) {
                if (isset($args['api']) && $args['api']->getSignatureVersion() == 'bearer') {
    ...
    public static function _apply_app_id($value, array &$args)
    {
        ...
        if (strlen($value) > $MAX_APP_ID_LENGTH) {
            trigger_error("The provided or configured value for `AppId`, " . "which is an user agent parameter, exceeds the maximum length of " . "{$MAX_APP_ID_LENGTH} characters.", \E_USER_WARNING);
        }
        $args['app_id'] = $value;
    }
    public static function _default_app_id(array $args)
    {
        return ConfigurationResolver::resolve('sdk_ua_app_id', '', 'string', $args);
    }
    public static function _apply_user_agent($inputUserAgent, array &$args, HandlerList $list): void
    {
        ...
        $list->appendBuild(Middleware::mapRequest(function (RequestInterface $request) use ($userAgent) {
    ...
    {
        return ConfigurationResolver::resolve('account_id_endpoint_mode', 'preferred', 'string', $args);
    }
    public static function _apply_account_id_endpoint_mode($value, array &$args)
    {
        ...
        if (!in_array($value, $accountIdEndpointModes)) {
            throw new IAE("The value provided for the config account_id_endpoint_mode is invalid." . "Valid values are: " . implode(", ", $accountIdEndpointModes));
        }
    ...
    public static function _default_endpoint_provider(array $args)
    {
        ...
        if (self::isValidService($serviceName) && self::isValidApiVersion($serviceName, $apiVersion)) {
            ...
            return new \BackdropS3FS\Aws\EndpointV2\EndpointProviderV2($ruleset, EndpointDefinitionProvider::getPartitions());
        }
        $options = self::getEndpointProviderOptions($args);
    ...
    public static function _apply_auth_scheme_preference(string|array|null &$value, array &$args): void
    {
    ...
    {
        if ($value) {
            $args['suppress_php_deprecation_warning'] = \true;
        } elseif (!empty(getenv("AWS_SUPPRESS_PHP_DEPRECATION_WARNING"))) {
            $args['suppress_php_deprecation_warning'] = \BackdropS3FS\Aws\boolean_value(getenv("AWS_SUPPRESS_PHP_DEPRECATION_WARNING"));
        } elseif (!empty($_SERVER["AWS_SUPPRESS_PHP_DEPRECATION_WARNING"])) {
            $args['suppress_php_deprecation_warning'] = \BackdropS3FS\Aws\boolean_value($_SERVER["AWS_SUPPRESS_PHP_DEPRECATION_WARNING"]);
        } elseif (!empty($_ENV["AWS_SUPPRESS_PHP_DEPRECATION_WARNING"])) {
            $args['suppress_php_deprecation_warning'] = \BackdropS3FS\Aws\boolean_value($_ENV["AWS_SUPPRESS_PHP_DEPRECATION_WARNING"]);
        }
    ...
    {
        if ($args['config']['ignore_configured_endpoint_urls'] || !self::isValidService($args['service'])) {
            return '';
        }
        $serviceIdentifier = \BackdropS3FS\Aws\manifest($args['service'])['serviceIdentifier'];
        $value = ConfigurationResolver::resolve('endpoint_url_' . $serviceIdentifier, '', 'string', $args + ['ini_resolver_options' => ['section' => 'services', 'subsection' => $serviceIdentifier, 'key' => 'endpoint_url']]);
        if (empty($value)) {
            $value = ConfigurationResolver::resolve('endpoint_url', '', 'string', $args);
        }
        if (!empty($value)) {
            $args['config']['configured_endpoint_url'] = \true;
        }
        return $value;
    }
...
EOT;
    ...
    {
        static $typeDefaultMap = ['int' => 0, 'bool' => \false, 'boolean' => \false, 'string' => ''];
        return ConfigurationResolver::resolve($key, $typeDefaultMap[$expectedType] ?? '', $expectedType, $args);
    }
    /**
    ...
    {
        if (isset($args['api']) && !empty($args['api']->getClientContextParams())) {
            $clientContextParams = $args['api']->getClientContextParams();
            foreach ($clientContextParams as $paramName => $paramDefinition) {
                $definition = ['type' => 'value', 'valid' => [$paramDefinition['type']], 'doc' => $paramDefinition['documentation'] ?? null];
    ...
    private static function isValidService($service)
    {
        ...
        $services = \BackdropS3FS\Aws\manifest();
