...
namespace BackdropS3FS\Aws\Glacier;

use BackdropS3FS\Aws\CommandInterface;
use BackdropS3FS\Aws\HashingStream;
use BackdropS3FS\Aws\Multipart\AbstractUploader;
use BackdropS3FS\Aws\Multipart\UploadState;
use BackdropS3FS\Aws\PhpHash;
use BackdropS3FS\Aws\ResultInterface;
use BackdropS3FS\GuzzleHttp\Psr7;
use BackdropS3FS\Psr\Http\Message\StreamInterface as Stream;
/**
...
class MultipartUploader extends AbstractUploader
{
    ...
    {
        parent::__construct($client, $source, $config + ['account_id' => '-', 'vault_name' => null]);
    }
    protected function loadUploadWorkflowInfo()
    {
        return ['command' => ['initiate' => 'InitiateMultipartUpload', 'upload' => 'UploadMultipartPart', 'complete' => 'CompleteMultipartUpload'], 'id' => ['account_id' => 'accountId', 'vault_name' => 'vaultName', 'upload_id' => 'uploadId'], 'part_num' => 'range'];
    }
    ...
    {
        list($rangeIndex, $rangeSize) = $this->parseRange($command['range'], $this->state->getPartSize());
        $this->state->markPartAsUploaded($rangeIndex, ['size' => $rangeSize, 'checksum' => $command['checksum']]);
    }
