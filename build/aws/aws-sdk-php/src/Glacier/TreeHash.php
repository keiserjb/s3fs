...
use BackdropS3FS\Aws\HashInterface;
/**
...
class TreeHash implements HashInterface
{
    ...
    public function update($data)
    {
        ...
        if ($this->hash) {
            throw new \LogicException('You may not add more data to a ' . 'complete tree hash.');
        }
        ...
        while (strlen($this->buffer) >= self::MB) {
            ...
            $this->checksums[] = hash($this->algorithm, $data, \true);
        }
        return $this;
    }
    /**
    ...
    public function addChecksum($checksum, $inBinaryForm = \false)
    {
        // Error if hash is already calculated
        if ($this->hash) {
            throw new \LogicException('You may not add more checksums to a ' . 'complete tree hash.');
        }
    ...
    public function complete()
    {
        if (!$this->hash) {
            ...
            if (strlen($this->buffer) > 0) {
                $this->checksums[] = hash($this->algorithm, $this->buffer, \true);
                $this->buffer = '';
            }
            ...
            while (count($hashes) > 1) {
                ...
                $hashes = array();
                foreach ($sets as $set) {
                    $hashes[] = count($set) === 1 ? $set[0] : hash($this->algorithm, $set[0] . $set[1], \true);
                }
            }
            $this->hash = $hashes[0];
        }
