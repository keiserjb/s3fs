...
namespace BackdropS3FS\Aws\Api\Serializer;

use BackdropS3FS\Aws\Api\ListShape;
use BackdropS3FS\Aws\Api\MapShape;
use BackdropS3FS\Aws\Api\Service;
use BackdropS3FS\Aws\Api\Operation;
use BackdropS3FS\Aws\Api\Shape;
use BackdropS3FS\Aws\Api\StructureShape;
use BackdropS3FS\Aws\Api\TimestampShape;
use BackdropS3FS\Aws\CommandInterface;
use BackdropS3FS\Aws\EndpointV2\EndpointV2SerializerTrait;
use BackdropS3FS\Aws\EndpointV2\Ruleset\RulesetEndpoint;
...
use BackdropS3FS\Psr\Http\Message\UriInterface;
/**
...
abstract class RestSerializer
{
    ...
    private static array $excludeContentType = ['s3' => \true, 'glacier' => \true];
    /** @var Service */
    private Service $api;
    /** @var Uri */
    private $endpoint;
    ...
    private function buildEndpoint(Operation $operation, array $args, array $opts): UriInterface
    {
        ...
        if ($this->api->getServiceName() === 's3' && isset($args['Key']) && $this->shouldPreservePath($args['Key'])) {
    ...
    private function expandUriTemplate(Operation $operation, array $args): string
    {
        ...
        return preg_replace_callback(self::TEMPLATE_STRING_REGEX, static function (array $matches) use ($varDefinitions) {
    ...
    private function hasPayloadParam(StructureShape $input, $payload)
    {
        if ($payload) {
            ...
            if (!empty($payloadMember['union']) || in_array($payloadMember['type'], $potentiallyEmptyTypes)) {
                return \false;
            }
        }
