...
use BackdropS3FS\Aws\Exception\InvalidJsonException;
use BackdropS3FS\Aws\Sdk;
use BackdropS3FS\GuzzleHttp\Exception\TransferException;
...
class InstanceProfileProvider
{
    ...
    public function __construct(array $config = [])
    {
        ...
        $this->client = $config['client'] ?? \BackdropS3FS\Aws\default_http_handler();
    ...
    private function request($url, $method = 'GET', $headers = [])
    {
        ...
        if (defined('BackdropS3FS\HHVM_VERSION')) {
            $userAgent .= ' HHVM/' . HHVM_VERSION;
        }
        $userAgent .= ' ' . \BackdropS3FS\Aws\default_user_agent();
    ...
    {
        $isImdsV1Disabled = \BackdropS3FS\Aws\boolean_value($this->ec2MetadataV1Disabled) ?? \BackdropS3FS\Aws\boolean_value(ConfigurationResolver::resolve(self::CFG_EC2_METADATA_V1_DISABLED, self::DEFAULT_AWS_EC2_METADATA_V1_DISABLED, 'bool', $this->config)) ?? self::DEFAULT_AWS_EC2_METADATA_V1_DISABLED;
