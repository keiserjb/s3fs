...
namespace BackdropS3FS\Aws\DynamoDb;

use BackdropS3FS\Aws\CommandInterface;
use BackdropS3FS\Aws\CommandPool;
...
class WriteRequestBatch
{
    ...
    {
        // Apply defaults
        $config += ['table' => null, 'batch_size' => 25, 'pool_size' => 1, 'autoflush' => \true, 'before' => null, 'error' => null];
    ...
    {
        $this->queue[] = ['table' => $this->determineTable($table), 'data' => ['PutRequest' => ['Item' => $item]]];
        $this->autoFlush();
        return $this;
    }
    /**
    ...
    {
        $this->queue[] = ['table' => $this->determineTable($table), 'data' => ['DeleteRequest' => ['Key' => $key]]];
        $this->autoFlush();
        return $this;
    }
    /**
     ...
     * @return $this
     */
    public function flush($untilEmpty = \true)
    {
    ...
    private function prepareCommands()
    {
        ...
        foreach ($batches as $batch) {
            ...
            $commands[] = $this->client->getCommand('BatchWriteItem', ['RequestItems' => $requests]);
        }
        return $commands;
    }
    /**
    ...
    private function retryUnprocessed(array $unprocessed)
    {
        foreach ($unprocessed as $table => $requests) {
            foreach ($requests as $request) {
                $this->queue[] = ['table' => $table, 'data' => $request];
            }
        }
    }
    /**
