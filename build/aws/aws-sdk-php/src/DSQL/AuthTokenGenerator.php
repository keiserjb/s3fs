...
class AuthTokenGenerator
{
    ...
    {
        if ($expiration <= 0) {
            throw new \InvalidArgumentException("Lifetime must be a positive number, was {$expiration}");
        }
        if (empty($region)) {
        ...
        $url = (string) $signer->presign($request, $provider()->wait(), '+' . $expiration . ' seconds')->getUri();
