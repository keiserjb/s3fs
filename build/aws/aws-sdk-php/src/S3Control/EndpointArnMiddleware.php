...
use BackdropS3FS\Aws\Exception\UnresolvedEndpointException;
use BackdropS3FS\Aws\S3\EndpointRegionHelperTrait;
...
class EndpointArnMiddleware
{
    ...
    private function validateBucketArn(ArnInterface $arn)
    {
        ...
        throw new InvalidArnException('Provided ARN was not a valid S3 bucket' . ' ARN.');
    }
    ...
    private function validateArn(ArnInterface $arn)
    {
        ...
        if ($arn instanceof OutpostsArnInterface && !empty($this->config['dual_stack'])) {
            throw new UnresolvedEndpointException('Dualstack is currently not supported with S3 Outposts ARNs.' . ' Please disable dualstack or do not supply an Outposts ARN.');
        }
        ...
        if (!$clientPart->isRegionMatch($this->region, 's3')) {
            $clientPart = $this->partitionProvider->getPartition(\BackdropS3FS\Aws\strip_fips_pseudo_regions($this->region), 's3');
        }
