...
namespace BackdropS3FS\Aws\EventBridge;
...
class EventBridgeEndpointMiddleware
{
        ...
        if (in_array($cmd->getName(), $sigV4aCommands)) {
            if (isset($cmd['EndpointId'])) {
                ...
                if (!$this->isCustomEndpoint) {
                    $dnsSuffix = $this->endpointProvider->getPartition($this->region, 'eventbridge')->getDnsSuffix();
    ...
    private function validateEndpointId($endpointID)
    {
        ...
        if ($this->config['use_fips_endpoint']) {
            throw new InvalidArgumentException("EventId is currently not compatible with FIPS pseudo regions");
        }
        if ($this->config['dual_stack']) {
            throw new InvalidArgumentException("EventId is currently not compatible with dualstack");
        }
    }
}
