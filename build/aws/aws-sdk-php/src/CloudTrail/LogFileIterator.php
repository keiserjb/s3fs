...
namespace BackdropS3FS\Aws\CloudTrail;

use BackdropS3FS\Aws\S3\S3Client;
use BackdropS3FS\Aws\CloudTrail\Exception\CloudTrailException;
/**
...
class LogFileIterator extends \IteratorIterator
{
    ...
    public static function forTrail(S3Client $s3Client, CloudTrailClient $cloudTrailClient, array $options = [])
    {
        ...
        try {
            $result = $cloudTrailClient->describeTrails(['trailNameList' => [$trailName]]);
    ...
    public function current()
    {
        if ($object = parent::current()) {
            return ['Bucket' => $this->s3BucketName, 'Key' => $object['Key']];
        }
        return \false;
    }
    /**
    ...
    private function buildListObjectsIterator(array $options)
    {
        ...
        // key prefix.
        $objectsIterator = $this->s3Client->getIterator('ListObjects', ['Bucket' => $this->s3BucketName, 'Prefix' => $logKeyPrefix]);
