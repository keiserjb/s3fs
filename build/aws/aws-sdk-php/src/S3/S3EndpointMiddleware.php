...
use BackdropS3FS\GuzzleHttp\Exception\InvalidArgumentException;
...
class S3EndpointMiddleware
{
    ...
    private function applyPathStyleEndpointCustomizations(CommandInterface $command, RequestInterface $request)
    {
        if ($command->getName() == 'WriteGetObjectResponse') {
            ...
            $fips = \BackdropS3FS\Aws\is_fips_pseudo_region($this->region) ? "-fips" : "";
            $region = \BackdropS3FS\Aws\strip_fips_pseudo_regions($this->region);
