...
namespace BackdropS3FS\Aws\S3\Parser;

use BackdropS3FS\Aws\Api\Service;
use BackdropS3FS\Aws\CommandInterface;
use BackdropS3FS\Aws\ResultInterface;
use BackdropS3FS\Aws\S3\CalculatesChecksumTrait;
use BackdropS3FS\Aws\S3\Exception\S3Exception;
...
final class ValidateResponseChecksumResultMutator implements S3ResultMutator
{
    ...
    public function __invoke(ResultInterface $result, ?CommandInterface $command = null, ?ResponseInterface $response = null): ResultInterface
    {
        ...
        $shouldSkipValidation = $this->shouldSkipValidation($mode, $checksumModeEnabled, $responseAlgorithms);
        if ($shouldSkipValidation) {
            return $result;
        }
        $checksumPriority = $this->getChecksumPriority();
        $checksumsToCheck = array_intersect($responseAlgorithms, array_map('strtoupper', array_keys($checksumPriority)));
    ...
    private function validateChecksum($checksumPriority, ResponseInterface $response): array
    {
        ...
        if (!empty($checksumToValidate)) {
            $checksumHeaderValue = $response->getHeaderLine('x-amz-checksum-' . $checksumToValidate);
    ...
    {
        return empty($responseAlgorithms) || $mode === 'when_required' && $checksumModeEnabled !== 'enabled';
    }
    /**
     * @return string[]
     */
    ...
    {
        return extension_loaded('awscrt') ? self::$supportedAlgorithms : array_slice(self::$supportedAlgorithms, 1);
    }
    /**
    ...
    private function isMultipartGetObject(CommandInterface $command, array $checksumValidationInfo): bool
    {
        ...
        return is_numeric($endOfChecksum) && (int) $endOfChecksum > 1 && (int) $endOfChecksum < 10000;
    }
}
